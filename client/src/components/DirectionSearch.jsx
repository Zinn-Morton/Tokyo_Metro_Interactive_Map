...
// My components
import { SearchbarWithIcon, SelectWithIcon } from "./SearchbarWithIcon.jsx";
...
const DirectionSearch = forwardRef(({ directionDropdown }, ref) => {
  ...
  // Expand search when no results shown and on desktop
  const expand_class = !is_results_shown && isBrowser ? "expand" : "";
  ...
  return (
    ...
    >
      ...
      >
        <form
          className={`directions-search-input-container ${expand_class}`}
          onSubmit={getDirections}
        >
          {/* Make space on mobile when search has already gone through */}
          {isMobile && is_results_shown ? (
            <>
            ...
            <>
              {/* Layout for desktop or search results not showing */}
              {/* Start input */}
